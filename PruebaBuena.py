            ...
            for hour in hours:
                while True:
                    ...
                    if teacher_assignment[subject] in teachers and subject_hours[subject] > 0:
                        ...
                        break  # Break the loop once a subject is assigned

                    # Check if the schedule is full
                    if all(subject_hours[subj] == 0 for subj in subject_hours):
                        schedule[(day, hour)] = '---'
                        break  # Break the loop if all subjects' hours are exhausted
...
def plot_schedule(schedule):
    ...
    # Plotting the schedule with different color tones
    plt.figure(figsize=(10, 6))
    plt.imshow(schedule_matrix, cmap='viridis', aspect='auto')
      
    # Add grid around each subject
    for i in range(len(hours)):
        for j in range(len(days)):
            rect = plt.Rectangle((j - 0.5, i - 0.5), 1, 1, fill=False, color='black', linewidth=1)
            plt.gca().add_patch(rect)

    # Add subject names as labels for each cell
    for i in range(len(hours)):
        for j in range(len(days)):
            plt.text(j, i, subjects[schedule_matrix[i][j]], ha='center', va='center', color='black',fontweight='bold', fontsize=12)
    ...
    for padre1, padre2 in zip(poblation1, poblation2):
        ...
        for materia in materias:
            if materia != '---':
                ...
                # Si un padre no tiene suficientes genes para una materia, rellenar los genes restantes con esa materia de manera aleatoria
                while len(genes_padre1) < hours_per_subject[materia]:
                    genes_padre1.append(random.choice([gene for gene, valor in padre1.items() if valor == '---']))
                while len(genes_padre2) < hours_per_subject[materia]:
                    genes_padre2.append(random.choice([gene for gene, valor in padre2.items() if valor == '---']))
                
                # Mezclar los genes
                random.shuffle(genes_padre1)
                random.shuffle(genes_padre2)
                
                # Asignar las horas a los hijos respetando la cantidad de horas por materia
                for i in range(hours_per_subject[materia]):
                    if i < len(genes_padre1):
                        hijo1[genes_padre1[i]] = materia
                    if i < len(genes_padre2):
...
if __name__ == "__main__":
    ...
    #Declare inputs
    teacher_max_hours = {'Pep': 5, 'Juan': 5, 'Cr7':4 , 'Puigdemont': 5, 'Francisco F.': 10}
    hours_per_subject = {'Math': 2, 'English': 3, 'Chemistry': 4, 'History': 2, 'Physics': 1}
